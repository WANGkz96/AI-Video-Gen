...
PORT="${PORT}" MODELS="${MODELS}" GENERATOR_BACKEND="${BACKEND}" GENERATOR_API_URL="${GENERATOR_API_URL}" AI_VIDEO_GEN_DOWNLOAD_COMFY_MODELS="${AI_VIDEO_GEN_DOWNLOAD_COMFY_MODELS}" AI_VIDEO_GEN_PROVISIONING_STATUS="${AI_VIDEO_GEN_PROVISIONING_STATUS}" AI_VIDEO_GEN_ENABLE_LTX="${AI_VIDEO_GEN_ENABLE_LTX}" AI_VIDEO_GEN_ENABLE_LONGCAT="${AI_VIDEO_GEN_ENABLE_LONGCAT}" CORS_ORIGINS="${CORS}" bash "${ROOT_DIR}/scripts/bootstrap_vast.sh"

mkdir -p "${ROOT_DIR}/.run"
if [ "${BACKEND}" = "comfyui-ltx23" ] && [ "${AI_VIDEO_GEN_ENABLE_LTX}" = "1" ]; then
...
if [ "${AI_VIDEO_GEN_ENABLE_LONGCAT}" = "1" ]; then
  nohup env HF_TOKEN="${HF_TOKEN:-}" \
    ...
    2> "${ROOT_DIR}/.run/longcat-provision.err.log" \
    < /dev/null &
